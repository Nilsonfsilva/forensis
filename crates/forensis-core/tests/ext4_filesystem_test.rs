...
fn build_table() -> Ext4BlockGroupTable {
    ...
    let descriptor_data = {
        ...
        bytes[0x00..0x04].copy_from_slice(
            &(descriptor.block_bitmap() as u32).to_le_bytes(),
        );
        bytes[0x04..0x08].copy_from_slice(
            &(descriptor.inode_bitmap() as u32).to_le_bytes(),
        );
        bytes[0x08..0x0C].copy_from_slice(
            &(descriptor.inode_table() as u32).to_le_bytes(),
        );
...
fn build_integration_image() -> MemoryReader {
    ...
    image[superblock_offset + 4..superblock_offset + 8]
        .copy_from_slice(&16u32.to_le_bytes());
    image[superblock_offset + 0x0C..superblock_offset + 0x10]
        .copy_from_slice(&8u32.to_le_bytes());
    image[superblock_offset + 0x10..superblock_offset + 0x14]
        .copy_from_slice(&8u32.to_le_bytes());
    image[superblock_offset + 0x14..superblock_offset + 0x18]
        .copy_from_slice(&0u32.to_le_bytes());
    image[superblock_offset + 0x18..superblock_offset + 0x1C]
        .copy_from_slice(&2u32.to_le_bytes());
    image[superblock_offset + 0x20..superblock_offset + 0x24]
        .copy_from_slice(&8u32.to_le_bytes());
    image[superblock_offset + 0x28..superblock_offset + 0x2C]
        .copy_from_slice(&8u32.to_le_bytes());
    ...
    image[descriptor_offset + 4..descriptor_offset + 8]
        .copy_from_slice(&11u32.to_le_bytes());
    image[descriptor_offset + 8..descriptor_offset + 12]
        .copy_from_slice(&12u32.to_le_bytes());
    image[descriptor_offset + 12..descriptor_offset + 14]
        .copy_from_slice(&4u16.to_le_bytes());
    image[descriptor_offset + 14..descriptor_offset + 16]
        .copy_from_slice(&4u16.to_le_bytes());
    image[descriptor_offset + 16..descriptor_offset + 18]
        .copy_from_slice(&1u16.to_le_bytes());

    // Group 1.
    let group1_offset = descriptor_offset + Ext4BlockGroupDescriptor::SIZE;

    image[group1_offset..group1_offset + 4]
        .copy_from_slice(&13u32.to_le_bytes());
    image[group1_offset + 4..group1_offset + 8]
        .copy_from_slice(&14u32.to_le_bytes());
    image[group1_offset + 8..group1_offset + 12]
        .copy_from_slice(&15u32.to_le_bytes());
    image[group1_offset + 12..group1_offset + 14]
        .copy_from_slice(&4u16.to_le_bytes());
    image[group1_offset + 14..group1_offset + 16]
        .copy_from_slice(&4u16.to_le_bytes());
    image[group1_offset + 16..group1_offset + 18]
        .copy_from_slice(&1u16.to_le_bytes());
...
fn creates_filesystem_with_valid_metadata() {
    let superblock =
        Ext4Superblock::parse(&build_superblock()).expect("superblock should parse");

    let table = build_table();

    let filesystem =
        Ext4Filesystem::new(superblock, table).expect("filesystem should be created");
...
fn accepts_single_block_group_table() {
    ...
    data[0x00..0x04].copy_from_slice(
        &(descriptor.block_bitmap() as u32).to_le_bytes(),
    );
    data[0x04..0x08].copy_from_slice(
        &(descriptor.inode_bitmap() as u32).to_le_bytes(),
    );
    data[0x08..0x0C].copy_from_slice(
        &(descriptor.inode_table() as u32).to_le_bytes(),
    );
    data[0x0C..0x0E].copy_from_slice(&10u16.to_le_bytes());
    data[0x0E..0x10].copy_from_slice(&2u16.to_le_bytes());
...
fn returns_valid_block_group() {
    ...
    let group = filesystem
        .block_group(0)
        .expect("block group should exist");
...
fn opens_filesystem_from_reader() {
    ...
    let filesystem =
        Ext4Filesystem::open(&mut ext4_reader).expect("filesystem should open");
...
fn reads_inode_table_through_filesystem() {
    ...
    let inode_one = Ext4Inode::parse(table.inode(0).expect("inode should exist"))
        .expect("inode should parse");

    assert!(inode_one.is_regular_file());

    let inode_two = Ext4Inode::parse(table.inode(1).expect("inode should exist"))
        .expect("inode should parse");
